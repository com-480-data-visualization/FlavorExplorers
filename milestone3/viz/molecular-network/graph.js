...
nodes.forEach( (node, i) => {
  ...
  // if(node.healthy_score <= 0.0){
  //   node.fx = Math.random() * width / 3 + 0.1 * width;
  // } else {
  //   node.fx = width_half + Math.random() * width_quarter;
  // }
...
node.append("circle")
  .attr("r", d => d3.max([d.num_compounds * circle_scale, circle_min_radius]))
